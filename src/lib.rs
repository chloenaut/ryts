...
async fn get_yt_data( url: String) -> Result<Document, reqwest::Error> {
...
pub async fn search_for_generic(query: &str, search_type: char) -> Result<ResponseList,reqwest::Error> {
	...
	let doc = get_yt_data(search_url).await?;
...
pub async fn get_playlist_videos(playlist_id: String) -> Result<ResponseList, reqwest::Error> {
	let mut result_list = ResponseList::new();
	let doc = get_yt_data(["http://www.youtube.com/playlist?list=", &playlist_id].concat()).await?;
	for node in doc.find(Name("script")) {
		let node_text = node.text();
		let scr_txt = get_json(&node_text);
		if scr_txt != None {
...
pub async fn get_channel_videos(channel_id: String) -> Result<ResponseList, reqwest::Error> {
	let mut result_list = ResponseList::new();
	let doc = get_yt_data(["https://www.youtube.com/channel/", &channel_id, "/videos"].concat()).await?;
	for node in doc.find(Name("script")) {
		let node_text = node.text();
		let scr_txt = get_json(&node_text);
		if scr_txt != None {		
...
pub async fn get_video_suggestions(video_id: String) -> Result<ResponseList, reqwest::Error> {
	let mut result_list = ResponseList::new();
	let doc = get_yt_data(["https://www.youtube.com/watch?v=", &video_id].concat()).await?;
	for node in doc.find(Name("script")) {
		let node_text = node.text();
		let scr_txt = get_json(&node_text);
		if scr_txt != None {
			let json: serde_json::Value = serde_json::from_str(scr_txt.unwrap()).unwrap();
			let search_contents = json["contents"]["twoColumnWatchNextResults"]
			["secondaryResults"]["secondaryResults"]["results"].as_array().unwrap();
			for i in 0..search_contents.len() {
				if search_contents[i].get("compactVideoRenderer") != None {
					let vid_id = search_contents[i]["compactVideoRenderer"]["videoId"].as_str().unwrap();
					let vid_title = search_contents[i]["compactVideoRenderer"]["title"]["simpleText"].as_str().unwrap();
					result_list.add_item(Item{id: vid_id.to_string(),name: vid_title.to_string(), item_type: "video".to_string()});
				}
